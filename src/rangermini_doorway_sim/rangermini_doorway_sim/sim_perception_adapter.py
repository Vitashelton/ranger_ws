...
class SimPerceptionAdapter(Node):
    def __init__(self):
        ...
        self.cloud_pub = self.create_publisher(
            PointCloud2, "/office_rpg/npc_pointcloud", 10)
        ...
        self.create_timer(0.05, self.publish_npc_cloud)
    ...
    def publish_npc_cloud(self):
        """Publish anonymous NPC surface returns as a lightweight 3-D sensor."""
        if self.robot is None:
            return
        points = []
        cy, sy = math.cos(self.robot_yaw), math.sin(self.robot_yaw)
        for wx, wy in self.truth_poses.values():
            dx, dy = wx - self.robot[0], wy - self.robot[1]
            center_x = cy * dx + sy * dy
            center_y = -sy * dx + cy * dy
            for angle_index in range(12):
                angle = 2.0 * math.pi * angle_index / 12.0
                for z in (0.15, 0.45, 0.75, 1.05):
                    points.append((center_x + 0.20 * math.cos(angle),
                                   center_y + 0.20 * math.sin(angle), z))
        if not points:
    ...
    def ranger_at_observation_point(self, region):
        ...
        return math.hypot(self.robot[0] - float(point["x"]),
                          self.robot[1] - float(point["y"])) <= float(
                              point.get("arrival_radius", 0.6)) + 0.12

...
class TaskExecutor(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(String, '/task_graph_verifier/verified_graph', self._cb, 10)
    ...
    def _start(self,req,res):
        ...
        for node in self.graph['nodes']:
            ...
            if node.get('action') == 'navigate' and self.nav_client and 'pose' in node.get('params', {}):
                goal = NavigateToPose.Goal(); goal.pose.header.frame_id = 'map'
                pose = node['params']['pose']; goal.pose.pose.position.x = float(pose['x']); goal.pose.pose.position.y = float(pose['y'])

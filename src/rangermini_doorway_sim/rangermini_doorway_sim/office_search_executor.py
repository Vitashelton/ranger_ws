...
class OfficeSearchExecutor(Node):
    def __init__(self):
        ...
        default_cfg = str(Path(get_package_share_directory(
            "rangermini_doorway_sim")) / "config" / "npc_schedules.yaml")
        ...
        self.create_subscription(String, "/office_rpg/planner_injection",
                                 self.on_planner_injection, 10)
    ...
    def reset_state(self):
        ...
        self.llm_latencies = []
        self.pending_planner_injection = ""
        self.gate_result = {
    ...
    def schedule_region_prior(self, person):
        # Only the public schedule prior is available here. Actual NPC schedule
        # state/pose remains behind the simulation perception boundary.
        prior_key = "default"
    ...
    def publish_graph(self):
        self.graph_pub.publish(String(data=json.dumps(
            self.graph.snapshot(self.phase), ensure_ascii=False)))

    def on_planner_injection(self, msg):
        """One-shot demo fault; still traverses the real Evidence Gate."""
        self.pending_planner_injection = msg.data
        self.emit("HALLUCINATION_INJECTION_ARMED",
                  "下一次语义规划将注入 room_999，用于验证 Evidence Gate")
    ...
    def on_spec(self, msg):
        ...
        armed_injection = self.pending_planner_injection
        self.reset_state()
        self.pending_planner_injection = armed_injection
        self.spec = spec
    ...
    def planner_request(self, graph=None):
        return {
            ...
            "dynamic_graph": graph or self.graph.snapshot(self.phase),
            "execution_history": list(self.execution_history[-20:]),
            "last_failure": self.current_failure,
        }
    ...
    def start_semantic_planning(self):
        if not self.graph_initialized:
            ...
            self.publish_graph()
        graph = self.graph.snapshot(self.phase)
        request = self.planner_request(graph)
        self.planning_token += 1
        token = self.planning_token
        provider_name = self.spec.get("llm_provider", "offline")
        test_response = self.pending_planner_injection or str(
            self.get_parameter("planner_test_response").value)
        self.pending_planner_injection = ""
        self.llm_debug = {
            "provider_requested": provider_name,
            "api_configured": False, "api_called": False,
    ...
    def consume_semantic_plan(self):
        ...
        current_graph = self.graph.snapshot(self.phase)
        self.llm_call_count += int(bool(debug.get("api_called")))
        if debug.get("latency_s") is not None:
            self.llm_latencies.append(float(debug["latency_s"]))
        if raw_plan is None:
            primary_gate = {
        ...
        self.gate_result = {
            **final_gate, "primary_result": primary_gate,
            "fallback_used": fallback_used,
            "result": "FALLBACK" if fallback_used else "ACCEPT",
            "display": (("REJECTED: " + primary_gate["rejected_reason"])
                        if fallback_used else "ACCEPT"),
            "final_decision": final_plan,
        }
        self.llm_debug_pub.publish(String(data=json.dumps(
            self.llm_debug, ensure_ascii=False)))
        self.gate_pub.publish(String(data=json.dumps(
            self.gate_result, ensure_ascii=False)))
        self.emit("LLM_DECISION", "语义规划调用与门控完成",
                  provider=debug.get("provider_used"), model=debug.get("model"),
                  latency_s=debug.get("latency_s"), decision=final_plan,
                  gate_result=self.gate_result.get("result"),
                  rejected_reason=primary_gate.get("rejected_reason", ""))
    ...
    def execute_semantic_action(self, plan):
        ...
        if action == "SEARCH_REGION":
            ...
            beliefs = next((item["candidates"]
                            for item in self.graph.snapshot(self.phase)["entity_beliefs"]
                            if item["entity_id"] == self.spec.get("target_person")), {})
            ranked = sorted(
    ...
    def generate_report(self):
        ...
        self.report = {
            ...
            "missed_checkpoints": list(dict.fromkeys(
                [p for p in self.patrol_checkpoints if p not in visited]
                + self.missed_checkpoints)),
        ...
        if self.phase == "PARSE_TASK":
            ...
            if mission_type == "NAVIGATE_TO_REGION":
                ...
                self.emit("SCHEDULE_TRIGGERED",
                          ...
                          scheduled_time=self.spec.get("scheduled_time"))
                self.start_next_patrol()
            elif mission_type in {"SEARCH_PERSON", "SEARCH_AND_NOTIFY",
                                  "SEARCH_NOTIFY_PATROL", "CONDITIONAL_PATROL"}:
            ...
            if self.spec.get("mission_type") == "SEARCH_NOTIFY_PATROL":
                self.patrol_checkpoints = list(self.spec.get("checkpoints") or ["room_908"])
                self.patrol_index = 0
                self.set_phase("PATROL_CHECKPOINT", "提醒完成，继续巡检908")
                self.start_next_patrol()
            else:
    ...
    def status(self):
        ...
        return {
            ...
            "planned_path": self.planned_path, "product": "EGA-OfficeNav",
            "graph_revision": self.graph.graph_revision,
            ...
            "llm_latencies": self.llm_latencies[-10:],
            "truth_access": "NONE",
        }

    ...
    image_transport::ImageTransport &it_)
{
  ...
  // Livox LiDAR publisher is RELIABLE + VOLATILE, so match it.
  rclcpp::QoS lidar_qos(rclcpp::KeepLast(5));
  lidar_qos.reliable();
  lidar_qos.durability_volatile();

  // Livox IMU publisher is also RELIABLE + VOLATILE.
  // Keep queue not too large, otherwise old IMU messages can accumulate.
  auto imu_qos = rclcpp::SensorDataQoS();
  imu_qos.keep_last(10);

  // Camera publisher is RELIABLE + TRANSIENT_LOCAL, but we request VOLATILE
  // to avoid receiving old latched frames. RELIABLE publisher can satisfy BEST_EFFORT subscriber.
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  // 保持原逻辑：等第一帧 lidar 到了以后再收 imu
  if (last_timestamp_lidar < 0.0) return;

  // 200Hz IMU 不建议每帧 INFO，会严重刷屏/拖慢
  // RCLCPP_INFO(this->node->get_logger(), "get imu at time: %.6f", stamp2Sec(msg_in->header.stamp));

  sensor_msgs::msg::Imu::SharedPtr msg(new sensor_msgs::msg::Imu(*msg_in));

  // 先应用 imu_time_offset
  double timestamp = stamp2Sec(msg->header.stamp) - imu_time_offset;

  // 检查 lidar 和 imu 是否在同一时间轴
  double lidar_imu_dt = last_timestamp_lidar - timestamp;

  if (fabs(lidar_imu_dt) > 0.5 && (!ros_driver_fix_en))
  {
    RCLCPP_WARN(
      this->node->get_logger(),
      "IMU and LiDAR not synced! lidar_time: %.9f, imu_time: %.9f, delta: %.9f",
  ...
  // 原 Fast-LIVO2 逻辑：修正 ros driver 的整数秒跳变
  // 但如果你不是明确需要这个功能，建议配置里先关掉 ros_driver_fix_en
  ...
  bool accept_this_imu = true;
  bool reset_imu_buffer = false;

  mtx_buffer.lock();

  if (last_timestamp_imu > 0.0)
  {
    double dt = timestamp - last_timestamp_imu;
    ...
    if (dt < -0.001)
    {
      RCLCPP_ERROR(
        this->node->get_logger(),
        "imu loop back, last: %.9f, current: %.9f, dt: %.9f. Clear imu buffer and reset.",
        last_timestamp_imu,
        timestamp,
        dt
      );

      reset_imu_buffer = true;
    }

    // IMU 时间向前大跳
    // 你的 IMU 是 200Hz，正常 dt 大约 0.005s
    // 超过 0.2s 就说明中间断流/积压/时间源跳变
    if (dt > 0.2)
    {
      RCLCPP_WARN(
        this->node->get_logger(),
        "imu time stamp jumps %.4lf seconds, last: %.9f, current: %.9f. Clear imu buffer and reset.",
        dt,
        last_timestamp_imu,
        timestamp
      );
  ...
  if (reset_imu_buffer)
  {
    ...
    // 关键：这里必须更新 last_timestamp_imu
    // 否则后续每一帧都会继续被判断为 jump，然后一直 return
    last_timestamp_imu = timestamp;

    // 当前这一帧作为新的起点放进去
    imu_buffer.push_back(msg);
  }
  else if (accept_this_imu)
  {
    last_timestamp_imu = timestamp;
    imu_buffer.push_back(msg);
  }

  // 不建议一直 cout，200Hz 会刷爆终端
...
if (img_cur.empty())
{
  RCLCPP_ERROR(this->node->get_logger(), "empty image from getImageFromMsg, skip this frame");
  mtx_buffer.unlock();
  sig_buffer.notify_all();
  return;
}
